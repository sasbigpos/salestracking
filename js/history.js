// history.js — Activity history table & detail modal
...
export class HistoryManager {
  ...
  async render() {
    this._all = await this.db.getActivities();
    this._filter();
  }
  ...
  _filter() {
    ...
    const filtered = this._all.filter(a => {
      const matchSearch = !search || a.clientName?.toLowerCase().includes(search) || a.activityType?.toLowerCase().includes(search) || a.notes?.toLowerCase().includes(search);
  ...
  _renderTable(activities) {
    ...
    if (!activities.length) {
      body.innerHTML = '<tr><td colspan="6" class="empty-td">No activities found.</td></tr>';
      return;
    }
    body.innerHTML = activities.map(a => `
      <tr>
        ...
        <td style="font-family:var(--font-mono);font-size:.75rem;color:var(--text2)">${a.lat ? `${a.lat.toFixed(4)}, ${a.lng.toFixed(4)}` : '<span style="color:var(--text3)">No GPS</span>'}</td>
        <td>
  ...
  async openDetail(id) {
    const activity = this._all.find(a => a.id === id) || (await this.db.getActivities()).find(a => a.id === id);
    if (!activity) return;

    const modal = document.getElementById('detailModal');
    document.getElementById('modalTitle').textContent = esc(activity.clientName);

    const questions = this.q ? this.db.getQuestions() : [];
    ...
    const customHtml = questions.map(q => {
      ...
      return `<div class="detail-row"><span class="detail-label">${esc(q.label)}</span><span class="detail-val">${esc(display)}</span></div>`;
    }).join('');
